defmodule OAuthServer.Client.Service do
  ...
  def whitelist_url(client_id, %URI{host: host, scheme: "https", path: path})
      when is_binary(client_id) and is_binary(host) do
    if exists?(client_id) do
      with {:ok, _} <-
             Repo.insert(%WhitelistORM{url: "https://" <> host <> path, client_id: client_id}) do
        :ok
      else
        err ->
          Logger.error("something went wrong whitelisting the url", error: inspect(err))
